  ...
  if (!history || history.length === 0) {
    return (
      <div className="p-6">
        ...
        >
          <p style={{ color: 'var(--text)' }}>You haven't taken any quizzes yet.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(bestByCategory).map(([category, percent]) => (
            <div
              key={category}
              className="rounded-xl shadow border p-5 flex flex-col items-center"
              style={{ 
                background: 'var(--card-bg)',
                borderColor: 'var(--border)'
              }}
            >
              ...
              <p className="text-xl font-semibold" style={{ color: 'var(--text)' }}>{percent}%</p>
            </div>
          ))}
        </div>
      </div>
        ...
        >
          <table className="w-full text-left">
            <thead style={{ background: 'var(--surface)' }}>
              <tr>
                <th className="p-3" style={{ color: 'var(--text)' }}>Category</th>
                <th className="p-3" style={{ color: 'var(--text)' }}>Score</th>
                <th className="p-3" style={{ color: 'var(--text)' }}>Percentage</th>
                <th className="p-3" style={{ color: 'var(--text)' }}>Date</th>
              </tr>
            </thead>
...
function SummaryCard({ label, value }) {
  return (
    <div className="rounded-2xl shadow border p-6 flex flex-col items-center"
      style={{ 
        background: 'var(--card-bg)',
        borderColor: 'var(--border)'
      }}
